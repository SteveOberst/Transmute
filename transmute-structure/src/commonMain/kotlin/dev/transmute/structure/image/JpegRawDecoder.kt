@file:Suppress("unused")

package dev.transmute.structure.image

import dev.transmute.codec.Decoder
import dev.transmute.common.PipelineContext
import dev.transmute.image.ImageFormat
import dev.transmute.model.core.Bytes
import dev.transmute.model.core.NoDecodeOptions
import dev.transmute.model.structure.image.JpegRaw

/** Decodes raw bytes into a [JpegRaw] structure. */
class JpegRawDecoder : Decoder<ImageFormat, JpegRaw, NoDecodeOptions> {
    private val reader = JpegStructureReader()
    override val decodableFormats: Set<ImageFormat> = setOf(ImageFormat.Jpeg)
    override fun sniff(data: Bytes): ImageFormat? =
        if (reader.canRead(data)) ImageFormat.Jpeg else null
    override suspend fun decode(source: Bytes, options: NoDecodeOptions, context: PipelineContext): JpegRaw =
        reader.read(source)
}
