...
import dev.transmute.model.metadata.itunes.ItunesTag
 ...
 */
internal fun IsoBmffBox.subBoxes(): List<IsoBmffBox> =
    if (data.data.isEmpty()) children else data.data.parseIsoBmffBoxes()
 ...
 */
internal fun IsoBmffBox.fullBoxSubBoxes(): List<IsoBmffBox> =
    if (data.size < 4) emptyList()
    else data.data.parseIsoBmffBoxes(offset = 4)
...
internal fun extractItunesMetadata(boxes: List<IsoBmffBox>): ItunesMetadata? {
    ...
    val tags = ilstChildren.mapNotNull { item -> parseItunesItem(item) }
    return if (tags.isEmpty()) null else ItunesMetadata(tags)
}

// -- iTunes ilst item parsing -------------------------------------------------

/**
 * Parse a single iTunes metadata item box into an [ItunesTag].
 *
 ...
 */
private fun parseItunesItem(item: IsoBmffBox): ItunesTag? {
    val key = item.type.value
    val dataBox = item.subBoxes().firstOrNull { it.type.value == "data" } ?: return null
    val d = dataBox.data.data
    if (d.size < 8) return null

    val flags = ((d[1].toInt() and 0xFF) shl 16) or
            ((d[2].toInt() and 0xFF) shl 8) or
            (d[3].toInt() and 0xFF)
    val payload = d.copyOfRange(8, d.size)

    val value = when (flags) {
        1 -> payload.decodeToString()   // UTF-8
        2 -> decodeUtf16(payload)        // UTF-16 BE
        0, 21 -> decodeItunesInteger(key, payload)
        13 -> "[JPEG artwork: ${payload.size} bytes]"
        14 -> "[PNG artwork: ${payload.size} bytes]"
        else -> "[binary: ${payload.size} bytes, flags=$flags]"
    }

    return ItunesTag(
        key = key,
        name = ITUNES_KEY_NAMES[key],
        dataType = flags,
        value = value,
    )
}
 ...
 */
private fun decodeItunesInteger(key: String, payload: ByteArray): String = when {
    // Track number: | padding (2B) | track (2B) | total (2B) | padding (2B) |
    (key == "trkn" || key == "disk") && payload.size >= 6 -> {
        val num = readBE16(payload, 2)
        val total = readBE16(payload, 4)
        if (total > 0) "$num/$total" else "$num"
    }
    payload.size == 1 -> (payload[0].toInt() and 0xFF).toString()
    payload.size == 2 -> readBE16(payload, 0).toString()
    payload.size == 4 -> readBE32(payload, 0).toString()
    payload.size == 8 -> readBE64(payload, 0).toString()
    else -> "[integer: ${payload.size} bytes]"
}

private fun decodeUtf16(payload: ByteArray): String {
    // Try to decode UTF-16 BE; fallback to ASCII
    if (payload.size < 2) return ""
    val chars = CharArray(payload.size / 2)
    for (i in chars.indices) {
        chars[i] = (((payload[i * 2].toInt() and 0xFF) shl 8) or
                (payload[i * 2 + 1].toInt() and 0xFF)).toChar()
    }
    return String(chars).trimEnd('\u0000')
}

private fun readBE16(d: ByteArray, off: Int): Int =
    ((d[off].toInt() and 0xFF) shl 8) or (d[off + 1].toInt() and 0xFF)

private fun readBE32(d: ByteArray, off: Int): Long =
...
private fun readBE64(d: ByteArray, off: Int): Long =
    (readBE32(d, off) shl 32) or (readBE32(d, off + 4) and 0xFFFFFFFFL)
 ...
 */
internal fun findHeifExifBytes(
    topLevelBoxes: List<IsoBmffBox>,
): ByteArray? {
...
private fun findItemLocation(ilocBox: IsoBmffBox, itemId: Int): Pair<Long, Long>? {
    ...
    val itemCount = if (version < 2) {
        val c = readBE16Iloc(d, pos); pos += 2; c
    } else {
        val c = readBE32Iloc(d, pos); pos += 4; c.toInt()
    }
    ...
    for (i in 0 until itemCount) {
        ...
        val id = if (version < 2) {
            val v = readBE16Iloc(d, pos); pos += 2; v
        } else {
            val v = readBE32Iloc(d, pos); pos += 4; v.toInt()
        }
        ...
        val baseOffset = readVariableInt(d, pos, baseOffsetSize); pos += baseOffsetSize

        // extent_count (2 bytes)
        val extentCount = readBE16Iloc(d, pos); pos += 2
        ...
        if (id == itemId && extentCount > 0) {
            ...
            val extentOffset = readVariableInt(d, pos, offsetSize); pos += offsetSize
