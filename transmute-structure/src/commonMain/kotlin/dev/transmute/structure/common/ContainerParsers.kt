 ...
 */
fun ByteArray.parseRiffChildren(
    offset: Int,
    end: Int,
): List<RiffChunk> {
 ...
 */
fun ByteArray.parseIsoBmffBoxes(
    offset: Int = 0,
    end: Int = size,
): List<dev.transmute.model.structure.common.IsoBmffBox> {
 ...
 */
fun ByteArray.parseEbmlElements(
    offset: Int = 0,
    end: Int = size,
): List<dev.transmute.model.structure.common.EbmlElement> {
 ...
 */
fun ByteArray.parseOggPages(
    offset: Int = 0,
    end: Int = size,
): List<dev.transmute.model.structure.common.OggPage> {
    val pages = mutableListOf<dev.transmute.model.structure.common.OggPage>()
    var pos = offset

    while (pos + 27 <= end) {
        // Sync to "OggS"
        if (this[pos] != 0x4F.toByte() || this[pos + 1] != 0x67.toByte() ||
            this[pos + 2] != 0x67.toByte() || this[pos + 3] != 0x53.toByte()
        ) {
            pos++
            continue
        }
...
internal fun ByteArray.readU16LE(off: Int): Int =
...
internal fun ByteArray.readU32LE(off: Int): UInt =
...
internal fun ByteArray.readU32BE(off: Int): UInt =
...
internal fun ByteArray.readU64BE(off: Int): ULong =
    ((this[off].toULong() and 0xFFu) shl 56) or
...
internal fun ByteArray.readI32LE(off: Int): Int =
...
internal fun ByteArray.readI64LE(off: Int): Long =
