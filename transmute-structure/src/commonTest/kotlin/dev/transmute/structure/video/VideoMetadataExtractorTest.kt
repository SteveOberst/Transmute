...
class VideoMetadataExtractorTest {
     ...
     */
    private fun isoBmffWithItunesTag(
        key: String = "\u00A9nam",
        value: String = "Test Title",
    ): List<IsoBmffBox> {
    ...
    fun webmExtractsMatroskaTags() {
        ...
        assertEquals("TITLE", tags.tags[0].simpleTags[0].name)
        assertEquals("My WebM Video", tags.tags[0].simpleTags[0].value)
    }

    @Test
    fun webmReturnsEmptyWithNoTags() {
        val segment = ebmlMaster(MatroskaIds.Segment, listOf(
    ...
    fun mkvExtractsMatroskaTags() {
        ...
        val tags = metadata.filterIsInstance<MatroskaTagMetadata>().firstOrNull()
        assertNotNull(tags)
        assertEquals("ARTIST", tags.tags[0].simpleTags[0].name)
        assertEquals("Test Director", tags.tags[0].simpleTags[0].value)
    }
    ...
    fun mp4ExtractsItunesMetadata() {
        ...
        assertEquals(1, itunes.items.size)
        assertEquals("aART", itunes.items[0].key)
        assertEquals("My MP4", itunes.items[0].value)
        assertEquals(1, itunes.items[0].dataType) // flags=1 = UTF-8
    }
    ...
    fun movExtractsItunesMetadata() {
        ...
        val itunes = metadata.filterIsInstance<ItunesMetadata>().firstOrNull()
        assertNotNull(itunes)
        assertEquals("Test Artist", itunes.items[0].value)
        assertEquals(1, itunes.items[0].dataType) // flags=1 = UTF-8
        assertEquals("Album Artist", itunes.items[0].name) // well-known name
    }
    ...
    fun aviExtractsRiffInfo() {
        ...
        assertEquals("INAM", info.entries[0].tag)
        assertEquals("Video", info.entries[0].value)
        assertEquals("IART", info.entries[1].tag)
        assertEquals("Director", info.entries[1].value)
    }
    ...
    fun m4aExtractsItunesMetadata() {
        ...
        val itunes = metadata.filterIsInstance<ItunesMetadata>().firstOrNull()
        assertNotNull(itunes)
        assertEquals("My Album", itunes.items[0].value)
        assertEquals(1, itunes.items[0].dataType) // flags=1 = UTF-8
    }
}
