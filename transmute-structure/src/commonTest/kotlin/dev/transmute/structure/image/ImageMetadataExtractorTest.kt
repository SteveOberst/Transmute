...
class ImageMetadataExtractorTest {
    ...
    fun pngExtractsTextChunks() {
        ...
        assertEquals(2, textMeta.entries.size)
        assertEquals("Title", textMeta.entries[0].keyword)
        assertEquals("My Image", textMeta.entries[0].text)
        assertEquals(PngTextChunkType.TEXT, textMeta.entries[0].chunkType)
        assertEquals("Author", textMeta.entries[1].keyword)
    }
    ...
    fun pngExtractsItxtChunks() {
        ...
        val textMeta = metadata.filterIsInstance<PngTextMetadata>().firstOrNull()
        assertNotNull(textMeta)
        assertEquals(1, textMeta.entries.size)
        assertEquals("Description", textMeta.entries[0].keyword)
        assertEquals("A test image", textMeta.entries[0].text)
        assertEquals(PngTextChunkType.ITXT, textMeta.entries[0].chunkType)
        assertEquals("en", textMeta.entries[0].language)
        assertEquals("Beschreibung", textMeta.entries[0].translatedKeyword)
    }

    @Test
    fun pngExtractsXmpFromItxt() {
        val xmpPayload = """<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>Test</dc:title></rdf:Description></rdf:RDF></x:xmpmeta>"""
        val png = PngRaw(
            signature = pngSignature,
            chunks = listOf(
        ...
        assertNotNull(xmp)
        assertEquals("xmpmeta", xmp.root.name)
        assertEquals("adobe:ns:meta/", xmp.root.namespace)

        // Verify the parsed tree structure: xmpmeta > RDF > Description > title
        val rdf = xmp.root.children.filterIsInstance<XmpNode.Element>().firstOrNull()
        assertNotNull(rdf, "expected RDF child element")
        assertEquals("RDF", rdf.element.name)
        ...
        assertEquals("Description", desc.element.name)
        ...
        assertEquals("title", title.element.name)
        ...
        assertTrue(textMeta.all { pm -> pm.entries.none { it.keyword == "XML:com.adobe.xmp" } })
    }
    ...
    fun jpegXmpExtraction() {
        ...
        val xmpPayload = """<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:creator>Tester</dc:creator></rdf:Description></rdf:RDF></x:xmpmeta>"""
        ...
        assertEquals("creator", creator?.element?.name)
    ...
    fun webpExtractsXmpChunk() {
        val xmpPayload = """<?xml version="1.0"?><x:xmpmeta xmlns:x="adobe:ns:meta/"><rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>WebP Test</dc:title></rdf:Description></rdf:RDF></x:xmpmeta>"""
        ...
        val rdfNode = xmp.root.children.filterIsInstance<XmpNode.Element>().firstOrNull()
        assertNotNull(rdfNode)
        assertEquals("RDF", rdfNode.element.name)
        ...
        assertEquals("title", titleNode.element.name)
