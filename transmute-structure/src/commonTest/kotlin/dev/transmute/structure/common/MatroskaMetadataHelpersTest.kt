...
class MatroskaMetadataHelpersTest {
    ...
    private fun tag(
        targetTypeValue: Int? = null,
        targetType: String? = null,
        simpleTags: List<EbmlElement> = emptyList(),
    ): EbmlElement {
    ...
    fun extractsSingleTagWithOneSimpleTag() {
        ...
        assertEquals("TITLE", result.tags[0].simpleTags[0].name)
        assertEquals("My Video", result.tags[0].simpleTags[0].value)
    }
    ...
    fun extractsMultipleSimpleTags() {
        ...
        assertEquals("ARTIST", result.tags[0].simpleTags[1].name)
        assertEquals("Author", result.tags[0].simpleTags[1].value)
    }
    ...
    fun extractsTargetInfo() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertEquals(50, result.tags[0].targetTypeValue)
        assertEquals("ALBUM", result.tags[0].targetType)
    }
    ...
    fun extractsMultipleTags() {
        ...
        assertEquals(50, result.tags[0].targetTypeValue)
        assertEquals(30, result.tags[1].targetTypeValue)
    }
    ...
    fun handlesTagWithBinaryData() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertEquals(128L, result.tags[0].simpleTags[0].binarySize)
    ...
    fun handlesTagLanguage() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertEquals("fre", result.tags[0].simpleTags[0].language)
    }
    ...
    fun simpleTagWithoutNameIsSkipped() {
        ...
        assertNotNull(result)
        assertEquals(1, result.tags[0].simpleTags.size)
        assertEquals("GOOD", result.tags[0].simpleTags[0].name)
    }
    ...
    fun targetWithoutTypeValueIsNull() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertNull(result.tags[0].targetTypeValue)
        assertEquals("TRACK", result.tags[0].targetType)
    }
    ...
    fun extractsNestedSimpleTags() {
        ...
        assertEquals("CHAPTER", st.name)
        assertEquals("Chapter 1", st.value)
        assertEquals(2, st.children.size)
        assertEquals("PART_NUMBER", st.children[0].name)
        assertEquals("1", st.children[0].value)
        assertEquals("PART_NAME", st.children[1].name)
        assertEquals("Introduction", st.children[1].value)
    ...
    fun extractsTargetTrackUIDs() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertEquals(listOf(12345L, 67890L), result.tags[0].trackUIDs)
        assertTrue(result.tags[0].editionUIDs.isEmpty())
        assertTrue(result.tags[0].chapterUIDs.isEmpty())
        assertTrue(result.tags[0].attachmentUIDs.isEmpty())
    }
    ...
    fun extractsTargetEditionAndChapterUIDs() {
        ...
        val result = extractMatroskaTags(elements)
        assertNotNull(result)
        assertEquals(listOf(111L), result.tags[0].editionUIDs)
        assertEquals(listOf(222L), result.tags[0].chapterUIDs)
        assertEquals(listOf(333L), result.tags[0].attachmentUIDs)
        assertTrue(result.tags[0].trackUIDs.isEmpty())
    }
