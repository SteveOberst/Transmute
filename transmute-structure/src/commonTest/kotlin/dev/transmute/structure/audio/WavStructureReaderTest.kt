...
class WavStructureReaderTest {
  ...
  private fun minimalWav(): ByteArray {
    ...
    p += 4
    "WAVE".encodeToByteArray().copyInto(out, p)
    p += 4
    // fmt chunk
    "fmt ".encodeToByteArray().copyInto(out, p)
    p += 4
    writeU32LE(out, p, fmtData.size)
    p += 4
    fmtData.copyInto(out, p)
    p += fmtData.size
    // data chunk
    "data".encodeToByteArray().copyInto(out, p)
    p += 4
    writeU32LE(out, p, sampleData.size)
    p += 4
