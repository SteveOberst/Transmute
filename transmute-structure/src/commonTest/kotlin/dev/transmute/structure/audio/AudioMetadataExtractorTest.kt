...
class AudioMetadataExtractorTest {
    ...
    fun mp3ExtractsId3v2Metadata() {
        ...
        assertNotNull(id3v2)
        assertEquals(3, id3v2.version.major)
        assertEquals(1, id3v2.frames.size)
        val content = id3v2.frames[0].content
    ...
    fun wavExtractsRiffInfo() {
        ...
        assertEquals("Test Song", info.entries[0].value)
        assertEquals("Title", info.entries[0].name)
        assertEquals("IART", info.entries[1].tag)
        assertEquals("Test Artist", info.entries[1].value)
        assertEquals("Artist", info.entries[1].name)
    }
    ...
    fun flacExtractsVorbisComment() {
        ...
        assertNotNull(vc)
        assertEquals("Lavf58.76", vc.vendor)
        assertEquals(2, vc.comments.size)
        assertEquals("TITLE", vc.comments[0].field)
        assertEquals("My FLAC Song", vc.comments[0].value)
        assertEquals("ALBUM", vc.comments[1].field)
        assertEquals("Test Album", vc.comments[1].value)
    }
    ...
    fun oggVorbisExtractsVorbisComment() {
        ...
        val vc = metadata.filterIsInstance<VorbisCommentMetadata>().firstOrNull()
        assertNotNull(vc)
        assertEquals(1, vc.comments.size)
        assertEquals("TITLE", vc.comments[0].field)
        assertEquals("Ogg Song", vc.comments[0].value)
    }
    ...
    fun opusExtractsVorbisComment() {
        ...
        val vc = metadata.filterIsInstance<VorbisCommentMetadata>().firstOrNull()
        assertNotNull(vc)
        assertEquals("ARTIST", vc.comments[0].field)
        assertEquals("Opus Artist", vc.comments[0].value)
    }
