...
import dev.transmute.codec.pipeline.TransformPipeline
...
class TransformPipelineTest {
    ...
    private open class AlphaTransform(override val id: TransformId = TransformId("alpha")) :
        Transform<String> {
        override suspend fun apply(ir: String, context: PipelineContext) = "$ir+alpha"
    }

    private open class BetaTransform(override val id: TransformId = TransformId("beta")) :
        Transform<String> {
        override suspend fun apply(ir: String, context: PipelineContext) = "$ir+beta"
    }

    private open class GammaTransform(override val id: TransformId = TransformId("gamma")) :
        Transform<String> {
        override suspend fun apply(ir: String, context: PipelineContext) = "$ir+gamma"
    }
    ...
    fun transformsExecuteInPipelineOrder() = runTest {
        ...
        var result = "start"
        for (t in p) {
            result = t.apply(result, PipelineContext(logger = TransmuteLogger.Noop))
        }
