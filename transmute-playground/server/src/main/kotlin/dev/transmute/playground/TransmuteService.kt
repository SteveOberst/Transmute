...
class TransmuteService(
  ...
  fun imageFormats() = formatList(MediaDomainDto.IMAGE, transmute.codec.image.decodableFormats, transmute.codec.image.encodableFormats) {
    it is ImageFormat.Unknown
  }
  fun audioFormats() = formatList(MediaDomainDto.AUDIO, transmute.codec.audio.decodableFormats, transmute.codec.audio.encodableFormats) {
    it is AudioFormat.Unknown
  }
  fun videoFormats() = formatList(MediaDomainDto.VIDEO, transmute.codec.video.decodableFormats, transmute.codec.video.encodableFormats) {
    it is VideoFormat.Unknown
  }
  ...
  fun allTransforms(): List<TransformInfo> = imageTransforms() + audioTransforms() + videoTransforms()
  ...
  fun getPlugin(key: String): PluginDescriptor? = listPlugins().firstOrNull { it.key == key }
  ...
  private fun deriveDomains(): Set<MediaDomainDto> = buildSet {
    ...
    if (img.decodableFormats.any { it !is ImageFormat.Unknown } ||
      img.encodableFormats.any { it !is ImageFormat.Unknown }
    ) {
      add(MediaDomainDto.IMAGE)
    }
    val aud = transmute.codec.audio
    if (aud.decodableFormats.any { it !is AudioFormat.Unknown } ||
      aud.encodableFormats.any { it !is AudioFormat.Unknown }
    ) {
      add(MediaDomainDto.AUDIO)
    }
    val vid = transmute.codec.video
    if (vid.decodableFormats.any { it !is VideoFormat.Unknown } ||
      vid.encodableFormats.any { it !is VideoFormat.Unknown }
    ) {
      add(MediaDomainDto.VIDEO)
    }
  }
  ...
  private fun <F : MediaFormat<*, *>> MutableList<String>.collectAddedLabels(decodable: Set<F>, encodable: Set<F>) {
    (decodable + encodable)
   ...
   */
  private fun parseParamValue(value: String, param: KParameter, transformId: String, paramName: String): Any? {
...
data class UploadedFile(val handle: String, val name: String, val size: Long, val file: File)
