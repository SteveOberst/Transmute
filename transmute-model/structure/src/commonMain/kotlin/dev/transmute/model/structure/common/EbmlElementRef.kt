 ...
 */
@Serializable
data class EbmlElementRef(
    val id: EbmlId,
    val range: ByteRange,
    val children: List<EbmlElementRef> = emptyList(),
)
...
private fun Long.toEbmlVint(): ByteArray {
    ...
    return when {
        ...
        v <= 0x07FFFFFFFE         -> byteArrayOf(((v shr 32) or 0x08).toByte(), (v shr 24).toByte(), (v shr 16).toByte(), (v shr 8).toByte(), v.toByte())
        v <= 0x03FFFFFFFFFE       -> byteArrayOf(((v shr 40) or 0x04).toByte(), (v shr 32).toByte(), (v shr 24).toByte(), (v shr 16).toByte(), (v shr 8).toByte(), v.toByte())
        v <= 0x01FFFFFFFFFFFE     -> byteArrayOf(((v shr 48) or 0x02).toByte(), (v shr 40).toByte(), (v shr 32).toByte(), (v shr 24).toByte(), (v shr 16).toByte(), (v shr 8).toByte(), v.toByte())
        else                      -> byteArrayOf(((v shr 56) or 0x01).toByte(), (v shr 48).toByte(), (v shr 40).toByte(), (v shr 32).toByte(), (v shr 24).toByte(), (v shr 16).toByte(), (v shr 8).toByte(), v.toByte())
    }
}
...
data class EbmlElement(
    ...
    override fun toBytes(): Bytes {
        ...
        val payload = if (children.isNotEmpty()) {
            ...
            var pos = 0
            for (part in parts) { part.copyInto(buf, pos); pos += part.size }
            buf
        } else {
            data.data
        }
