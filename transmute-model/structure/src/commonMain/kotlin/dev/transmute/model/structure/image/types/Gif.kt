 ...
 */
@Serializable
data class GifColor(val red: UByte, val green: UByte, val blue: UByte) : BinarySerializable {
 ...
 */
@Serializable
data class GifImageDescriptor(val left: UShort, val top: UShort, val width: UShort, val height: UShort, val packed: UByte) {
...
data class GifRaw(
  ...
  override fun toBytes(): Bytes {
    ...
    var pos = 0
    sig.copyInto(out, pos)
    pos += sig.size
    lsd.data.copyInto(out, pos)
    pos += lsd.size
    gctBytes.copyInto(out, pos)
    pos += gctBytes.size
...
val GifRaw.imageDescriptors: List<GifImageDescriptor>
  get() = blocks
    ...
    .map { block ->
      ...
      fun readU16(off: Int): UShort = ((d[off + 1].toUInt() and 0xFFu) shl 8 or (d[off].toUInt() and 0xFFu)).toUShort()
      GifImageDescriptor(
        left = readU16(0),
        top = readU16(2),
        width = readU16(4),
