 ...
 */
@Serializable
data class BmpColorEntry(val blue: UByte, val green: UByte, val red: UByte, val reserved: UByte = 0u) : BinarySerializable {

  override fun toBytes(): Bytes = byteArrayOf(blue.toByte(), green.toByte(), red.toByte(), reserved.toByte()).asBytes()
...
data class BmpRaw(
  ...
  override fun toBytes(): Bytes {
    ...
    var pos = 0
    fh.data.copyInto(out, pos)
    pos += fh.size
    dh.data.copyInto(out, pos)
    pos += dh.size
