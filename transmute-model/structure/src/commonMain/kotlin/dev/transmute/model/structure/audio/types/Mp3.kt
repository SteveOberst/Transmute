 ...
 */
@Serializable
data class Mp3VbrInfo(
    val totalFrames: Int? = null,
    val totalBytes: ByteLength? = null,
...
data class Mp3Raw(
    ...
    override fun toBytes(): Bytes {
        ...
        var pos = 0
        tag2.copyInto(out, pos); pos += tag2.size
        audio.copyInto(out, pos); pos += audio.size
...
val Mp3Raw.vbrInfo: Mp3VbrInfo?
    get() {
        ...
        if (tag == "Xing" || tag == "Info") {
            fun be32(o: Int): UInt =
                (d[o].toUInt() and 0xFFu shl 24) or (d[o + 1].toUInt() and 0xFFu shl 16) or
            ...
            if (flags and 0x01u != 0u && d.size >= off + 4) { frames = be32(off).toInt(); off += 4 }
            if (flags and 0x02u != 0u && d.size >= off + 4) { bytes = ByteLength(be32(off).toLong()); off += 4 }
            if (flags and 0x04u != 0u) off += 100 // skip 100-entry TOC
            if (flags and 0x08u != 0u && d.size >= off + 4) { quality = be32(off).toInt() }
