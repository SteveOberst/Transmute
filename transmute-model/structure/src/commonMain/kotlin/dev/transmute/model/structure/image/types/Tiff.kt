...
data class TiffIfdEntry(
  ...
  override fun toBytes(): Bytes {
    ...
    out[0] = (tag.toInt() shr 8).toByte()
    out[1] = tag.toByte()
    out[2] = (fieldType.toInt() shr 8).toByte()
    out[3] = fieldType.toByte()
    val c = count
    out[4] = (c shr 24).toByte()
    out[5] = (c shr 16).toByte()
    out[6] = (c shr 8).toByte()
    out[7] = c.toByte()
...
data class TiffRaw(
  ...
  override fun toBytes(): Bytes {
    ...
    val ifdParts = ifds.map { ifd ->
      ...
      var pos = 0
      count.copyInto(buf, pos)
      pos += count.size
      for (eb in entryBytes) {
        eb.copyInto(buf, pos)
        pos += eb.size
      }
    ...
    var pos = 0
    boBytes.copyInto(out, pos)
    pos += 2
    magic.copyInto(out, pos)
    pos += 2
    ifdOff.copyInto(out, pos)
    pos += 4
    for (part in ifdParts) {
      part.copyInto(out, pos)
      pos += part.size
    }
    imageData.data.copyInto(out, pos)
    pos += imageData.size
