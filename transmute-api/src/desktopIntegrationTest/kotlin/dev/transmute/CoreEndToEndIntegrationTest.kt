 ...
 */
class CoreEndToEndIntegrationTest {
  ...
  private fun solidColor(width: Int, height: Int, r: Int = 128, g: Int = 128, b: Int = 128, a: Int = 255): ImageIR {
  ...
  fun jpeg_realMedia_structureReaderAccepts() = runTest {
    ...
    assertTrue(structure.headerSegments.isNotEmpty(), "JPEG must have header segments")
    assertNotNull(structure.frame, "JPEG must have a frame")
  }
  ...
  fun gif_realMedia_structureReaderAccepts() = runTest {
    ...
    // GIF blocks may be empty for a solid-color image with no extensions.
  }
  ...
  fun webp_realMedia_structureReaderAccepts() = runTest {
    ...
    val canEncodeWebp =
      ImageIO.getImageWritersByFormatName("webp")
        ...
        .firstOrNull() != null
    if (!canEncodeWebp) {
      println("SKIP webp_realMedia_structureReaderAccepts: WebP writer not available")
      return@runTest
    }
