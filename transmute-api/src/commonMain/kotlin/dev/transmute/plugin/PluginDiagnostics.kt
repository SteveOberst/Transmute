...
class AggregateDiagnostics internal constructor() {
  ...
  internal fun forPlugin(key: PluginId): PluginDiagnostics = _plugins.getOrPut(key) { PluginDiagnostics(key) }
  ...
  fun summary(): Map<PluginId, Boolean> = _plugins.mapValues { (_, diag) -> diag.current?.available ?: true }
}
