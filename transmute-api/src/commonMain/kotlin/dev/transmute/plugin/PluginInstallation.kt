 ...
 */
internal class PluginInstallation<C : Any>(val plugin: TransmutePlugin<C>, private val configure: C.() -> Unit) {
 ...
 */
internal fun sortPluginInstallations(installations: List<PluginInstallation<*>>): List<PluginInstallation<*>> {
  if (installations.isEmpty()) return installations
  ...
  // Nothing to sort once dependencies are validated.
  ...
  for ((k, deg) in inDegree) {
    if (deg == 0) queue.add(k)
  }
