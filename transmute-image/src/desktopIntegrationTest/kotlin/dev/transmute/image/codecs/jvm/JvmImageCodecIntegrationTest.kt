 ...
 */
class JvmImageCodecIntegrationTest {

  private val decoder = JvmImageIoDecoder()
  private val encoder = JvmImageIoEncoder()
  private val ctx = testContext()

  private suspend fun encodePng(ir: ImageIR): Bytes = encoder.encode(ir, ImageFormat.Png, CanonicalImageEncodeOptions(), ctx)
  ...
  fun jpegLowQualityHasMoreArtifacts() = runTest {
    ...
    val highDecoded = decoder.decode(highEncoded, CanonicalImageDecodeOptions(), ctx)
    val lowDecoded = decoder.decode(lowEncoded, CanonicalImageDecodeOptions(), ctx)
