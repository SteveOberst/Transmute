...
class CrossFormatPipelineTest {
    ...
    fun video_decodeResizeReencode_mp4ToMkv() = runTest {
        requireGStreamer {
            ...
            val final_ = mkv.decode(mkvBytes, CanonicalVideoDecodeOptions(), ctx)
            assertNotNull(final_.videoTrack, "Decoded resized MKV must have a video track")
            assertTrue(
                final_.videoTrack.width == resizedWidth,
                "Resized width should be $resizedWidth, got ${final_.videoTrack.width}",
            )
            assertTrue(
                final_.videoTrack.height == resizedHeight,
                "Resized height should be $resizedHeight, got ${final_.videoTrack.height}",
            )
        }
    }
