...
class InMemoryFileSystem : TransmuteFileSystem {
  ...
  override fun metadata(path: TPath): FileMetadata = metadataOrNull(path) ?: throw FileNotFoundException("Not found: $path", path)

  override fun metadataOrNull(path: TPath): FileMetadata? = when (val node = nodes[key(path)]) {
  ...
  override fun openWrite(path: TPath, mode: WriteMode): WriteHandle = InMemoryWriteHandle(this, path, mode)
...
private class InMemoryWriteHandle(private val fs: InMemoryFileSystem, private val path: TPath, private val mode: WriteMode) : WriteHandle {
