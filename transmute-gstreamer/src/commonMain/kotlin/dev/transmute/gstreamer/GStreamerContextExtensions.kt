 ...
 * GStreamer additions for the enabled domains. If GStreamer is not installed
 * on the current system, only native platform codecs are registered.
 *
 * ```kotlin
 * val ctx = TransmuteContext {
 *     gstreamer {
 *         audio = true  // AAC, M4A, Opus, FLAC enc, OGG enc
