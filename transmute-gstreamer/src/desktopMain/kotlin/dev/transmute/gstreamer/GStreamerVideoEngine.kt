        ...
        try {
            ...
            if (audio != null && tmpAudio != null) {
                ...
                val wavBytes = wavEncoder.encode(
                    audioIR, AudioFormat.Wav, CanonicalAudioEncodeOptions(), context,
                ).data
            ...
            val args = buildList {
                ...
                add("filesrc"); add("location=${tmpFrames.absolutePath.toGstPath()}")
                add("!"); add("rawvideoparse")
                add("format=rgba"); add("width=$width"); add("height=$height")
                add("framerate=$fpsNum/1")
                add("!"); add("videoconvert")
                add("!"); add(videoEncoder)
                addAll(extraElements)
                add("!"); add("$muxElement"); add("name=mux")
                add("!"); add("filesink"); add("location=${tmpOut.absolutePath.toGstPath()}")

                // Audio input
                if (tmpAudio != null && audioEncoder != null) {
                    add("filesrc"); add("location=${tmpAudio.absolutePath.toGstPath()}")
                    add("!"); add("wavparse")
                    add("!"); add("audioconvert")
                    add("!"); add(audioEncoder)
                    add("!"); add("mux.")
                }
            }
